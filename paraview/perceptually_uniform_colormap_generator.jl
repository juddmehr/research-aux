...
"""
function diverging_map(s,rgb1,rgb2)
    colormap = zeros(length(s),3);
    for i=1:length(s)
        colormap[i,:] = diverging_map_1val(s[i],rgb1,rgb2);
    end
    return colormap
end

"""
Interpolate a diverging color colormap.
s is a number between 0 and 1
"""
function diverging_map_1val(s, rgb1, rgb2)
...
"""
For the case when interpolating from a saturated color to an unsaturated color, find a hue for the unsaturated color that makes sense.
"""
function AdjustHue(msh, unsatM)
